...
import com.example.web_client_lib.logging.ApiCallDetails
...
class WebClientWrapper(private val webClient: WebClient) {
    ...
    private fun validationException(error: WebClientResponseException): ValidationException {
        val errorResponse = ObjectMapper().readValue(error.responseBodyAsString, ValidationErrorDetails::class.java)
    ...
    private fun deserializeToMap(error: WebClientResponseException) =
        ObjectMapper().readValue(error.responseBodyAsString, Map::class.java) as Map<String, Any>

    private fun deserializeToErrorResponse(error: WebClientResponseException) =
        ObjectMapper().readValue(error.responseBodyAsString, ErrorResponse::class.java)
